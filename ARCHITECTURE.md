...
- **Selection** = structure-fit × win-rate confidence × expectancy multiplier (regime-specific). Expectancy is `pnl/n` in R (was mislabeled "%" before session 16j). `applyLearning(sim,auto)` updates per-strategy wins/losses/pnl + records confidence before→after, logs a rich entry, posts to the shared pool. Used by both `aiSelfBacktest(rounds)` (manual) and `aiAutoLearnOnce()` (idle auto).
