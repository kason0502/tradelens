...
- `roadmap.md` — the phased development roadmap (foundation → visual overhaul → AI intelligence → backtesting power → monetization → production). `TODO.md` carries the granular near-term items. The ONLY build system is the 5-agent pipeline below — no external orchestrators (the owner rejected "Hermes"; don't reintroduce it).
